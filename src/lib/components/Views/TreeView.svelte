<script lang="ts">
	...
	let childLimits = $state(new Map<string, number>());
	...
	function loadMoreChildren(parentPath: JSONPath): void {
		const pathKey = pathToKey(parentPath);
		const currentLimit = childLimits.get(pathKey) || 100;
		childLimits.set(pathKey, currentLimit + 100);
		childLimits = new Map(childLimits);
	}
	...
	function buildVisibleNodes(): FlatNode[] {
		...
		function walk(value: JSONValue, path: JSONPath = [], depth = 0): void {
			...
			if (shouldDescend && childCount > 0) {
				...
				const limit = childLimits.get(pathKey) || 100;
				const limitedKeys = childrenKeys.slice(0, limit);

				if (isArray) {
					limitedKeys.forEach((i) => walk((value as JSONValue[])[i as number], [...path, i], depth + 1));
				} else if (isObject) {
					limitedKeys.forEach((key) => walk((value as Record<string, any>)[key as string], [...path, key], depth + 1));
				}
		...
		{#each visibleNodes as node (pathToKey(node.path))}
			{#if node.key === '...'}
				...
				>
					...
					<button
						class="flex items-center space-x-2 rounded-md bg-gray-100 px-3 py-2 text-xs font-medium text-gray-600 transition-all duration-200 hover:bg-gray-200"
						onclick={() => loadMoreChildren(node.path.slice(0, -1))}
						type="button"
					>
						<span>Load {Math.min(100, node.childCount)} more...</span>
						<span class="text-gray-400">({node.childCount} remaining)</span>
					</button>
				</div>
			{:else}
				<div
